...
import { Plus, Check, X, BedDouble, UserPlus, Search, Pencil, Trash2, User, Stethoscope } from 'lucide-react'
import type { Appointment, AppointmentStatus, Patient, Doctor } from '@/types'
...
const admitSchema = z.object({
  ...
  nurse_id: z.string().min(1, 'নার্স নির্বাচন করুন'),
})
...
export default function AppointmentsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: AptForm) => {
      ...
      return api.manager.createAppointment(hospitalId, {
        ...
        doctor_id:  data.doctor_id,  doctor_name:  doctor.name,
  ...
  const admitMutation = useMutation({
    mutationFn: (data: AdmitForm) => api.manager.admitPatient(admitApt!.id, data),
    onSuccess: () => {
      invalidate()
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">মোট {appointments.length}টি অ্যাপয়েন্টমেন্ট</p>
        </div>
              ...
              {showWalkIn && !selectedPatient && (
                <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3 items-end">
                    <Select label="রক্তের গ্রুপ (ঐচ্ছিক)" placeholder="অজানা"
      ...
      <Modal
        isOpen={!!admitApt}
        onClose={() => { setAdmitApt(null); admitForm.reset() }}
      ...
      >
        <form onSubmit={admitForm.handleSubmit(d => admitMutation.mutate(d))} className="space-y-4">
          ...
          />
          <Select label="নার্স নির্বাচন করুন"
            error={admitForm.formState.errors.nurse_id?.message}
            options={availableNurses.map(n => ({ value: n.id, label: `${n.name} — ${n.ward}` }))}
            placeholder="নার্স নির্বাচন করুন"
            {...admitForm.register('nurse_id')}
          />
          <div className="flex justify-end gap-3 pt-2">
            <Button type="button" variant="ghost" onClick={() => { setAdmitApt(null); admitForm.reset() }}>বাতিল</Button>
